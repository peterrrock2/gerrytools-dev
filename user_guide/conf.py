"""Sphinx configuration for the GerryTools documentation."""
...
# Make user_guide/_pygments_warm.py importable for the dotted pygments_style paths below.
sys.path.insert(0, os.path.dirname(__file__))

# -- Project information -----------------------------------------------------

project = "GerryTools"
copyright = "2018-2026, Data and Democracy Research Initiative"
author = "Data and Democracy Research Initiative"

release = package_version("gerrytools")
...
intersphinx_mapping = {
    ...
    "gerrychain": ("https://gerrychain.readthedocs.io/en/latest/", None),
}

templates_path = ["_templates"]
# Static assets and jupyter_execute are not documentation sources.
exclude_patterns = ["_build", "_static", "jupyter_execute", "Thumbs.db", ".DS_Store"]
language = "en"
master_doc = "index"
...
#
#     DOCS_SWITCHER=1 DOCS_PALETTE=blush-blue task docs-serve
...
htmlhelp_basename = "GerryToolsdoc"
...
# docs-serve (sphinx-autobuild) watches user_guide/, so an unconditional write would retrigger
...
# Render ``Attributes`` sections as field lists. Sphinx otherwise creates one target from the
# Napoleon section and another for the same dataclass member during autodoc traversal.
napoleon_use_ivar = True

# -- linkcheck ---------------------------------------------------------------

# The plan-comparison optimizer's source docstring uses a ``DOI:...`` pseudo-URI. Sphinx's link
# checker cannot resolve that scheme, while the DOI remains visible in the API page.
linkcheck_ignore = [r"DOI:10\.1109/TAES\.2016\.140952"]
