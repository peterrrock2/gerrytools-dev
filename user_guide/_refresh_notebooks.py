...
Usage:
    python user_guide/_refresh_notebooks.py [NOTEBOOK ...]

With no arguments every notebook under ``user_guide/user/`` is checked. Notebooks missing from the
cache run in a fresh kernel inside a temporary working directory. Executed notebooks are stored in
MyST-NB's cache under ``user_guide/_build/``; committed notebooks remain output-free and
reviewable.
"""
...
def execute(path: Path) -> nbformat.NotebookNode:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        cwd = root / "user"
        cwd.mkdir()
        shutil.copytree(DOCS / "_static" / "data", cwd / "data")
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        nargs="*",
        type=Path,
        help="notebooks to cache (default: every notebook under user_guide/user/)",
    )
    parser.add_argument(
        "--force",
        action="store_true",
    ...
    notebooks = args.notebooks or sorted(TUTORIALS.rglob("*.ipynb"))
    if not notebooks:
